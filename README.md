...
Veritas is a prediction market demo built on top of the Rialo protocol. It lets you create markets, place bets, and watch them resolve — all without a real blockchain, real tokens, or real oracles. The point is to show what Rialo actually feels like to use before the mainnet exists.

---

## What is Rialo?

Most prediction markets today rely on oracles — trusted third parties that report real-world outcomes on-chain. Rialo removes that dependency entirely. Instead of asking a reporter "did X happen?", a Rialo market fetches the answer directly from the internet at resolution time. You specify which URLs to read and how to interpret them when you create the market. When the deadline passes, the contract hits all those sources simultaneously, runs a consensus check, and settles — no human in the loop.

Veritas is the testnet demo of that idea. Everything runs in your browser. The "chain" is localStorage. The tokens are fake. But the mechanics — market creation, betting, multi-source resolution, consensus, payouts, disputes — are real and fully wired up.
...
Click **Connect Wallet** in the top right. You'll get a choice of three pre-loaded mock wallets, each starting with 10,000 RIALO. Pick one and you're in. Your address, balance, and transaction history persist across page reloads.
...
Open any market that's still accepting bets. Pick the outcome you believe in, enter an amount (minimum 100 RIALO), and confirm. Your bet goes into that outcome's pool. Probabilities update in real time as the pools shift. A 2% protocol fee is deducted at resolution, not at bet time.

### Creating a market

Hit **Create Market** and fill in:

- **Question** — what you're asking (up to 280 characters)
...
If your outcome won, a **Claim** button appears on your position. Your payout is proportional to your share of the winning pool, drawn from the entire pot minus the protocol fee. Disputed markets show a **Refund** button instead — you get your original stake back in full.
...
- **`src/lib/mock/contract.ts`** — the core contract functions: `createMarket`, `placeBet`, `triggerResolution`, `claimPayout`, `claimRefund`. Each function validates state, mutates the Zustand store, and submits a mock transaction
- **`src/lib/mock/wallet.ts`** — deterministic address generation from a label string, balance management, and wallet registry persistence
- **`src/lib/mock/sources.ts`** — source adapters with JSON path extraction and outcome mapping

All persistent state (markets, positions, resolution logs, wallet balances, transaction history) is written to `localStorage` under the keys `veritas-market-store` and `veritas-wallet-store`.

### State management

State is managed with Zustand. There are three stores:
...
**Pool-based pricing.** There are no order books. Each outcome has a pool. When you bet on an outcome, your tokens go into that pool. Your implied probability is `outcomePool / totalPool`. This is the same constant-product style used by most on-chain prediction markets.
...
**Consensus threshold.** Each market has a `consensus_threshold` (default: 3). Resolution succeeds only if at least that many sources agree on the same outcome. If the threshold isn't met — because sources disagree, return errors, or produce a tie — the market disputes and all positions become refundable.
...
Open `http://localhost:3000`. No environment variables are required — everything runs client-side.
